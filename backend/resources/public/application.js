  ...
  $(function() {
    ...
    window.IdeaList = Backbone.Collection.extend({
      ...
      comparator: function(idea) {
        return idea.get("hotness");
      },
      refresh: function() {
        var ideas;
        ideas = this;
        return $.get("/ideas", function(data) {
          var idea, ideaData, ideasData, newIdeas;
          ideasData = eval(data);
          newIdeas = [];
          ...
          return _.each(newIdeas, function(newIdea) {
            return Ideas.add(newIdea);
          });
        });
      },
      initialize: function() {
    ...
    return window.Ideas = new IdeaList;
  });
...
(function() {
  ...
  $(function() {
    return window.IdeaView = Backbone.View.extend({
      tagName: "li",
      events: {
      ...
      updateHotness: function() {
        var hotness, opacity;
        hotness = this.model.get("hotness");
        $(".hotness .value", this.el).css({
          width: hotness + "%"
        });
        opacity = hotness < 10 ? 0.1 : hotness / 100.0;
        return $(this.el).css({
          opacity: opacity
        });
      },
      resetHotness: function() {
        ...
        return this.updateHotness();
      },
      setModel: function(model) {
        this.model = model;
        this.model.bind("change", this.updateHotness, this);
        return this.ideaId = this.model.ideaId;
      }
    });
  });
  ...
  $(function() {
    window.AppView = Backbone.View.extend({
      ...
      addOne: function(idea) {
        ...
        idea.bind("change:hotness", function(idea, newHotness) {
          return App.reorder(idea, newHotness, view);
        });
        return this.$("#ideas").prepend(view.render().el);
      },
      reorder: function(idea, newHotness, view) {
        var element;
